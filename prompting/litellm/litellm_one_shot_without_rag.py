...
def generate_response_test(test_sample, train_valid_discharge) -> dict:

    test_phase_2_discharge_target = pd.read_csv("/nethome/vdquoc/project_dsai/data/test_phase_2/discharge_target.csv")

    ##### Filtered hadm_id for oneshot prompting with/without rag ######
    samples_with_length_smaller_8192 = pd.read_csv("/nethome/vdquoc/project_dsai/output_files/list_smaller_8192_test_tv_without_multiturn.csv")
    ...
    for index, row in tqdm(test_sample.iterrows()):
        ...
        if (samples_with_length_smaller_8192['hadm_id'].isin([hadm_id]).any()):
            ...
            dict_to_csv['generated_discharge_instruction'].append(response_di['choices'][0]['message']['content'])
            ...
            dict_to_csv['generated_brief_hospital_course'].append(response_bhc['choices'][0]['message']['content'])
...
if __name__ == '__main__':
    ...
    output = generate_response_test(test_sample=test_sample, train_valid_discharge=train_valid_discharge)
    df = pd.DataFrame.from_dict(output)
    df.to_csv('/nethome/vdquoc/project_dsai/output_files/output_one_shot_without_rag_test_tv.csv')
    print('time of version one_shot_without_rag on TEST_PHASE_2 with train_valid combined')
