...
def generate_response_test(test_sample, test_retrieved_hadm_id_tv, train_valid_discharge) -> dict:
    ...
    for index, row in tqdm(test_sample.iterrows()):
        ...
        retrieved_hadm_id = test_retrieved_hadm_id_tv.loc[row['hadm_id'] == test_retrieved_hadm_id_tv['hadm_id'], 'retrieved_hadm_id'].iloc[0]
        ...
        sample_input_data['json'] = sample_input_data.apply(lambda x: x.to_json(), axis=1)
        ...
        tokenized_prompt_di = encode(model="anyscale/meta-llama/Meta-Llama-3-70B-Instruct", text=prompt_di)
        tokenized_prompt_bhc = encode(model="anyscale/meta-llama/Meta-Llama-3-70B-Instruct", text=prompt_bhc)
        print(len(prompt_di))
        print(len(prompt_bhc))

        if len(tokenized_prompt_di) < 8192 and len(tokenized_prompt_bhc) < 8192:
            print(row['hadm_id'])
            print('--------------------------')
            messages_di = [
            ...
            correcting_prompt_bhc = f'''
                ...
                '''
            tokenized_correcting_prompt_di = encode(model="anyscale/meta-llama/Meta-Llama-3-70B-Instruct", text=correcting_prompt_di)
            tokenized__correcting_prompt_bhc = encode(model="anyscale/meta-llama/Meta-Llama-3-70B-Instruct", text=correcting_prompt_bhc)
            print(len(tokenized_correcting_prompt_di))
            print(len(tokenized__correcting_prompt_bhc))

            if len(tokenized_correcting_prompt_di) < 8192 and len(tokenized__correcting_prompt_bhc) < 8192:
                dict_to_csv['hadm_id'].append(row['hadm_id'])
                i +=1
    
    print(i)
    print('number of hadm_ids after filtering',test_sample.shape[0] - i)
    return dict_to_csv
...
if __name__ == '__main__':
    ...
    df.to_csv('/nethome/vdquoc/project_dsai/output_files/filtered_hadm_ID_for_1_turn.csv')
